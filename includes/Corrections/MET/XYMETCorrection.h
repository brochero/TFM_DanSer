...
std::vector<double> METXYCorr_Met_MetPhi(double originalMet, double originalMet_phi, int runnb, int year, bool isMC, int npv){

  std::vector<double> METCorr = {originalMet,originalMet_phi,0.,0.};
  ...
  METCorr.at(0) = CorrectedMET;
  METCorr.at(1) = CorrectedMETPhi;
  METCorr.at(2) = METxcorr;
  METCorr.at(3) = METycorr;
  return METCorr;
}
