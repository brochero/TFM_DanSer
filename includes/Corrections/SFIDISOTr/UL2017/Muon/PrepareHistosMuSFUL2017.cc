...
void MergeHistos(TString File1, TString File3, TString Histo){
  ...
  if(File3.Contains("TriggerSF")){    
    ...
    // Not needed for UL SF: There are two options: eta and |eta|    
    // TH2F *HTotFullEtaRange =  HistoMirror(H_1);
    TH2F *HTotFullEtaRange =  (TH2F*)H_1->Clone();
    ...
    // Not needed for UL SF: Already set to |eta|(X) Vs pT (Y)
    // TH2F *HTotInv             = SwitchXYAxis(H_1); 
    TH2F *HTotInv             = (TH2F*)H_1->Clone(); 
...
void PrepareHistosMuSFUL2017(){

  // For ID
  MergeHistos("Efficiencies_muon_generalTracks_Z_Run2017_UL_ID.root","Mu_TightIDSF.root",
	      "NUM_TightID_DEN_TrackerMuons_abseta_pt_syst");
  // For ISO
  MergeHistos("Efficiencies_muon_generalTracks_Z_Run2017_UL_ISO.root","Mu_TightISOSF.root",
  	      "NUM_TightRelIso_DEN_TightIDandIPCut_abseta_pt_syst");

  // For Trigger
  MergeHistos("Efficiencies_muon_generalTracks_Z_Run2017_UL_SingleMuonTriggers.root","Mu_TriggerSF.root",
  	      "NUM_IsoMu27_DEN_CutBasedIdTight_and_PFIsoTight_eta_pt_syst");
