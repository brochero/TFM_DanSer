...
int main(int argc, const char *argv[])
{
  ...
  if (WFraction != "none")
  {
    ...
    if (WFraction == "F0")
      nWFraction = 0;
    else if (WFraction == "FL")
      nWFraction = 1;
    else if (WFraction == "FR")
      nWFraction = 2;
  ...
  if (fname.Contains("Data"))
  {
    ...
    std::cout << "Sample identified as MC." << std::endl;

  // For Data and QCD
  bool IsMu = true, IsEl = true;
  if (fname.Contains("DataSinleElectron") ||
      fname.Contains("EMEnriched"))
    IsMu = false;
  if (fname.Contains("DataSingleMuon") ||
      fname.Contains("MuEnriched"))
    IsEl = false;
  ...
  struct stat st;
  if (stat(SkimOutDir, &st) != 0)
    system("mkdir " + SkimOutDir);
  ...
  tree_R->Branch("j_eta", &j_eta, "j_eta/F");

  tree_R->Branch("l_pT", &l_pT, "l_pT/F");
  tree_R->Branch("l_eta", &l_eta, "l_eta/F");

  tree_R->Branch("DR_lj", &DR_lj, "DR_lj/F");
  tree_R->Branch("Deta_lj", &Deta_lj, "Deta_lj/F");
  tree_R->Branch("DpT_metj", &DpT_metj, "DpT_metj/F");
  tree_R->Branch("DPhi_lj", &DPhi_lj, "DPhi_lj/F");

  tree_R->Branch("Deta_lmet", &Deta_lmet, "Deta_lmet/F");
  //tree_R->Branch("Dphi_lmet", &Dphi_lmet, "Dphi_lmet/F");
  tree_R->Branch("DpT_lj", &DpT_lj, "DpT_lj/F");

  tree_R->Branch("DR_metj", &DR_metj, "DR_metj/F");
  //tree_R->Branch("DR_metjb", &DR_metjb, "DR_metjb/F");

  tree_R->Branch("DPhi_metj", &DPhi_metj, "DPhi_metj/F");
  tree_R->Branch("M_lj", &M_lj, "M_lj/F");
  ...
  tree_W->Branch("j_eta", &j_eta, "j_eta/F");

  tree_W->Branch("l_pT", &l_pT, "l_pT/F");
  tree_W->Branch("l_eta", &l_eta, "l_eta/F");

  tree_W->Branch("DR_lj", &DR_lj, "DR_lj/F");
  tree_W->Branch("Deta_lj", &Deta_lj, "Deta_lj/F");
  tree_W->Branch("DpT_metj", &DpT_metj, "DpT_metj/F");
  tree_W->Branch("DPhi_lj", &DPhi_lj, "DPhi_lj/F");

  tree_W->Branch("Deta_lmet", &Deta_lmet, "Deta_lmet/F");
  //tree_W->Branch("Dphi_lmet", &Dphi_lmet, "Dphi_lmet/F");
  tree_W->Branch("DpT_lj", &DpT_lj, "DpT_lj/F");

  tree_W->Branch("DR_metj", &DR_metj, "DR_metj/F");
  //tree_W->Branch("DR_metjb", &DR_metjb, "DR_metjb/F");

  tree_W->Branch("DPhi_metj", &DPhi_metj, "DPhi_metj/F");
  tree_W->Branch("M_lj", &M_lj, "M_lj/F");
  ...
  for (int ifile = 0; ifile < InputNameFiles.size(); ifile++)
  {
    ...
    for (Long64_t iEvt = 0; iEvt < NTreeEvtToRun; iEvt++)
    {
      ...
      if ((VetoMuons.size() + VetoElectrons.size()) > 1)
        continue;
      ...
      double mcWeight = 1.;
      if (SkimTree->genInfo_LHEWOrgXWGTUP != 0.)
        mcWeight = SkimTree->genInfo_LHEWOrgXWGTUP / std::abs(SkimTree->genInfo_LHEWOrgXWGTUP);
      ...
      if ((LepID_top == 0 && LepID_topbar == 0) || (LepID_top != 0 && LepID_topbar != 0))
        continue;
      else
      {
        if (LepID_top != 0)
        {
          Genchannel = std::abs(LepID_top) == 11 ? ch::ej : ch::mj;
          GenID = -1 * LepID_top / std::abs(LepID_top);
        }
        if (LepID_topbar != 0)
        {
          Genchannel = std::abs(LepID_topbar) == 11 ? ch::ej : ch::mj;
          GenID = -1 * LepID_topbar / std::abs(LepID_topbar);
        }
      }

      // Matching GenLepton and RecoLepton
      if (ichannel != Genchannel)
        continue;
      ...
      for (int ij = 0; ij < Jets.size(); ij++)
      {
        ...
        j_eta = ijet.Eta();
        l_pT = SeleLepton.Pt();
        l_eta = SeleLepton.Eta();

        DR_lj = ijet.DeltaR(SeleLepton);
        Deta_lj = ijet.Eta()-SeleLepton.Eta();
        ...
        //Et_met = MET.Et();
        //lmet_pT = (SeleLepton + MET).Pt();
        //W_mT_sq = pow(Et_met+SeleLepton.Et(), 2) - pow(MET.Pt()+SeleLepton.Pt(), 2);
        //W_mT = (MET+SeleLepton).Mt();
        M_lj = (ijet+SeleLepton).M();


        // if (j_Deep <= 0.4941) //limit (soft/hard)
        // {
        //   for (int kl = 0; kl < Jets.size(); kl++)
        //   { 
        //     NanoObj::Jet kjet = Jets.at(kl);
        //     if (kjet.DeepCSV <= 0.4941)
        //     {
        //       float M = (ijet+kjet).M();
        ...
        //         printf("same jet.\n");
        //       }
        //       if (M > 60 && M < 90) //btw the W mass
        //       { 
        //         printf("M is between 60 and 90.\n");
        //         //nW++
                
        //       } else 
        //       {
        //         printf("M is not between 60 and 90.\n");
        //       }
        //     }
        //   }
        // }
    ...
    std::cout << "--- End of event loop for sample " << ifile << ": ";
    sw.Print();
    std::cout << std::endl;
    sw.Continue();
...
int FindSysID(const std::vector<TString> Names, TString Name, TString BaseName)
{
  ...
  {
    if (Name == (BaseName + Names.at(iName)))
      return iName;
  }
  return -99;
}
